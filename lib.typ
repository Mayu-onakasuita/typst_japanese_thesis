...
#let ntnu-thesis(
  ...
  set page(
    paper: paper-size,
    margin: (bottom: 4.5cm, top:4cm, left:2cm, right: 2cm),
  )
  // Cover page
  if titlepage {
  ...
  if table-of-contents != none {
    ...
    set outline(indent: true, depth: 3, title: [目次])
    table-of-contents
  }
  ...
  show figure.where(kind: image): set figure(numbering: numbering-fig, supplement: [図])
  show figure.where(kind:table): set figure(numbering: numbering-fig, supplement: [表])
  let numbering-eq = n => {
    let h1 = counter(heading).get().first()
    numbering("(1.1)", h1, n)
  }
...
//Style appendix
#let appendix(chapters-on-odd: true, body) = {
